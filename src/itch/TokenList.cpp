...
void TLDeleteByType(TokenList* tl, int8_t dtype) {
	...
	while (didx < tl->size) {
		...
		while ( (didx + 1) < tl->size) {
			...
			// blank out the the one just moved
			tl->list[didx+1].id = 0;
			tl->list[didx+1].type = 0;
			tl->list[didx+1].label = NULL;
			didx++;
		}
		// reduce size
		tl->size--;
		if(tl->current > (tl->size - 1)) { tl->current = tl->size -1; }
		// check for next
		didx = _FindByType(tl, dtype);
	}
}
...
void TLDumpList(TokenList* tl) {
	...
	Serial.print("TokenList size: ");
	Serial.println(tl->size);
	Serial.print("TokenList current: ");
	Serial.println(tl->current);
	for (uint8_t i = 0; i < tl->size; i++ ) {
		Serial.print(i);
		Serial.print(": id:");
		Serial.print(tl->list[i].id);
		Serial.print(" type:");
		Serial.print(tl->list[i].type);
		Serial.print(" label:");
		Serial.println(tl->list[i].label);
	}
