...
Identifiers::Identifiers() {
    ...
    write_ident_lookup_code = false;		// set by lexer to include / exclude code sections
    write_lookup_lookup_code = false;		// set by lexer to include / exclude code sections
    write_action_calling_code = false;		// set by lexer to include / exclude code sections
}
...
void Identifiers::WriteXLATArrayOfTypeAndInstance(int type, char* array_instance_name) {
	...
	output.EnQueue(out);
	sprintf(out, "#endif //PLATFORM_ARDUINO\n");
	output.EnQueue(out);
	// entries
	xlat_idx = 0;
	idx = 0;
...
void Identifiers::WriteIdentMemberLookupCase(int case_num, char* string_array_instance, char* terminating_member_name) {
	...
	sprintf(out, "				#endif //PLATFORM_ARDUINO\n");
	output.EnQueue(out);
	sprintf(out, "				if(strnlen(temp.text, MAX_AST_IDENTIFIER_SIZE) == str_len) {\n");
	output.EnQueue(out);
...
void Identifiers::WriteLookupMemberLookupFunction(int header_or_code) {
	...
	sprintf(out, "uint8_t LookupLookupMembers(uint16_t ident_xlat, char* lookup_string, uint8_t str_len) {\n");
	output.EnQueue(out);
	sprintf(out, "	char str_terminated[MAX_AST_IDENTIFIER_SIZE];\n");
	output.EnQueue(out);
	sprintf(out, "	strncpy(str_terminated, lookup_string, MAX_AST_IDENTIFIER_SIZE);\n");
	output.EnQueue(out);
	sprintf(out, "	str_terminated[((str_len < MAX_AST_IDENTIFIER_SIZE) ? str_len : MAX_AST_IDENTIFIER_SIZE)] = '\\0';\n");
	output.EnQueue(out);
	sprintf(out, "	switch(ident_xlat) {\n");
	output.EnQueue(out);
	...
	while (idx < idents_count) {
		...
		if (ids[idx].Type == ID_LOOKUP_LIST) {
			...
			if (header_or_code == WRITE_HEADER) {
				...
				sprintf(out, "uint8_t %s(char* lookup_string) {\n", ids[idx].InstanceName);
				output.EnQueue(out);
				sprintf(out, "\t// Modify to suit. Default: call through to a function with %%user-code-call-through\n");
				output.EnQueue(out);
				sprintf(out, "\t// prefixed to the same name as this lookup function. Default \"My\"\n\n");
				output.EnQueue(out);
				if (user_code_call_through_commented) {
					sprintf(out, "\t//if (%s%s(lookup_string) != 0) return 1;\n", user_code_call_through, ids[idx].InstanceName);
					output.EnQueue(out);
					sprintf(out, "\t// NOTE: the following forces a positive lookup return if %%user-code-call-through-commented\n");
					output.EnQueue(out);
					sprintf(out, "\t// has been specified. Delete the next line when un-commenting the above line to make operative.\n");
					output.EnQueue(out);
					sprintf(out, "\treturn 1;\n");
					output.EnQueue(out);
